...
- (WeiboRequestOperation *)queryTimeline:(StatusTimeline)timeline
    ...
    if (timeline == StatusTimelineMentions) {
        queryPath = @"statuses/mentions.json";
          NSLog(@"timeline%d",timeline);
    }else if (timeline == StatusTimelineFriends){
        //
            NSLog(@"timeline%d",timeline);
        
        queryPath = @"statuses/friends_timeline.json";
    }else if (timeline == StatusTimelineUser){
        NSLog(@"timeline%d",timeline);

                  queryPath = @"statuses/user_timeline.json";
    }
//     queryPath = @"statuses/friends_timeline.json";
...
-(WeiboRequestOperation *)getNearby:(float)longfitude latitude:(float)latitude
                          completed:(WeiboTimelineQueryCompletedBlock)completedBlock{

     NSString *path = @"place/nearby/pois.json";
    
    NSString *longString = [NSString stringWithFormat:@"%f",longfitude];
    ...
    WeiboRequestOperation *operation =
    [[WeiboRequest shared]getFromPath:path params:params completed:^(id result, NSData *data, NSError *error) {
        if (error) {
            completedBlock(nil, error);
        }
        else {
//            NSMutableArray *statuses = [self pareseTimelineResponseObject:result];
            NSMutableArray *statuses = [result objectForKey:@"pois"];
